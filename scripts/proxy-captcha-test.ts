/**
 * Test with a ProxyScrape proxy — solve captcha WITH proxy
 */
import { config } from 'dotenv';
config({ path: '/home/z/my-project/.env' });
import * as fs from 'fs';

const LOG = '/home/z/my-project/scripts/proxy-test.log';
...
async function main() {
  ...
  const { getNextProxy, refreshProxyPool, getProxyStatus } = await import('../src/lib/proxy');
  ...
  try {
    const refresh = await refreshProxyPool();
    log(`Refresh: added=${refresh.added}, validated=${refresh.validated}, total=${refresh.total}`);
  } catch (e) {
    log(`Refresh failed: ${(e as Error).message}`);
  }

  const status = getProxyStatus();
  log(`Pool: ${status.poolSize} proxies, rotating=${status.isRotating}`);

  const proxy = await getNextProxy();
  if (!proxy) {
    log('No proxy available! Trying proxyless as fallback...');
  } else {
    log(`Proxy: ${proxy.host}:${proxy.port} (${proxy.country || '?'}, ${proxy.source || '?'}, ${proxy.anonymity || '?'})`);
  }
  ...
  if ((res.needsCaptcha || !res.success) && isCaptchaConfigured()) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      log(`Captcha attempt ${attempt}/3 (proxy=${proxy ? proxy.host : 'proxyless'})...`);
      const t0 = Date.now();
